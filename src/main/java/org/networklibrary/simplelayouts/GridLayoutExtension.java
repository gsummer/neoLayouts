...
public class GridLayoutExtension extends ServerPlugin {
	...
	public Iterable<Integer> gridlayout( @Source GraphDatabaseService graph )
	{
		...
		List<Integer> result = new ArrayList<Integer>(n*3);
		...
		try (Transaction tx = graph.beginTx()){
			for(Node node : GlobalGraphOperations.at(graph).getAllNodes()){
				...
				int currX = currCol * distanceBetweenNodesX;
				int currY = currRow * distanceBetweenNodesY;

				result.add(i, new Long(node.getId()).intValue());
				i=i+1; // go to x
				result.add(i, currX);
				i=i+1;
				result.add(i, currY);
